#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <ctype.h>
#include <stdarg.h>

#include "rtl.h"
#include "tac.h"
#include "symbol_table.h"

typedef struct StringMap
{
    char *literal;
    char *label;
    struct StringMap *next;
} StringMap;

static StringMap *string_map_head = NULL;
static int next_string_id = 0;

static char *xstrdup(const char *s)
{
    size_t n;
    char *p;

    if (!s)
        s = "";

    n = strlen(s) + 1;
    p = (char *)malloc(n);
    if (!p)
    {
        fprintf(stderr, "Out of memory in RTL generator\n");
        exit(1);
    }

    memcpy(p, s, n);
    return p;
}

static void append_text(char **dest, const char *text)
{
    size_t old_len;
    size_t add_len;
    char *buffer;

    if (!text || !*text)
        return;

    if (!*dest)
    {
        *dest = xstrdup(text);
        return;
    }

    old_len = strlen(*dest);
    add_len = strlen(text);
    buffer = (char *)realloc(*dest, old_len + add_len + 1);
    if (!buffer)
    {
        fprintf(stderr, "Out of memory in RTL generator\n");
        exit(1);
    }

    memcpy(buffer + old_len, text, add_len + 1);
    *dest = buffer;
}

static void append_linef(char **dest, const char *fmt, ...)
{
    va_list args;
    va_list copy;
    int len;
    char *line;

    va_start(args, fmt);
    va_copy(copy, args);
    len = vsnprintf(NULL, 0, fmt, copy);
    va_end(copy);

    if (len < 0)
    {
        va_end(args);
        fprintf(stderr, "Formatting error in RTL generator\n");
        exit(1);
    }

    line = (char *)malloc((size_t)len + 1);
    if (!line)
    {
        va_end(args);
        fprintf(stderr, "Out of memory in RTL generator\n");
        exit(1);
    }

    vsnprintf(line, (size_t)len + 1, fmt, args);
    va_end(args);

    append_text(dest, line);
    append_text(dest, "\n");
    free(line);
}

static void trim(char *s)
{
    size_t len;
    size_t i = 0;

    if (!s)
        return;

    while (s[i] && isspace((unsigned char)s[i]))
        i++;
    if (i > 0)
        memmove(s, s + i, strlen(s + i) + 1);

    len = strlen(s);
    while (len > 0 && isspace((unsigned char)s[len - 1]))
    {
        s[len - 1] = '\0';
        len--;
    }
}

static int is_integer_literal(const char *s)
{
    size_t i = 0;

    if (!s || !*s)
        return 0;

    if (s[i] == '+' || s[i] == '-')
        i++;

    if (!isdigit((unsigned char)s[i]))
        return 0;

    for (; s[i] != '\0'; i++)
    {
        if (!isdigit((unsigned char)s[i]))
            return 0;
    }

    return 1;
}

static int is_float_literal(const char *s)
{
    size_t i = 0;
    int seen_digit = 0;
    int seen_dot = 0;

    if (!s || !*s)
        return 0;

    if (s[i] == '+' || s[i] == '-')
        i++;

    for (; s[i] != '\0'; i++)
    {
        if (isdigit((unsigned char)s[i]))
        {
            seen_digit = 1;
            continue;
        }

        if (s[i] == '.')
        {
            if (seen_dot)
                return 0;
            seen_dot = 1;
            continue;
        }

        return 0;
    }

    return seen_digit && seen_dot;
}

static int is_float_reg(const char *reg)
{
    return reg && reg[0] == 'f';
}

static int is_string_literal(const char *s)
{
    size_t len;

    if (!s)
        return 0;

    len = strlen(s);
    return len >= 2 && s[0] == '"' && s[len - 1] == '"';
}

static const char *intern_string_label(const char *literal)
{
    StringMap *cur = string_map_head;
    char name[32];
    StringMap *node;

    while (cur)
    {
        if (strcmp(cur->literal, literal) == 0)
            return cur->label;
        cur = cur->next;
    }

    node = (StringMap *)malloc(sizeof(StringMap));
    if (!node)
    {
        fprintf(stderr, "Out of memory in RTL generator\n");
        exit(1);
    }

    snprintf(name, sizeof(name), "_str_%d", next_string_id++);
    node->literal = xstrdup(literal);
    node->label = xstrdup(name);
    node->next = string_map_head;
    string_map_head = node;

    return node->label;
}

static void free_string_map(void)
{
    StringMap *cur = string_map_head;

    while (cur)
    {
        StringMap *next = cur->next;
        free(cur->literal);
        free(cur->label);
        free(cur);
        cur = next;
    }

    string_map_head = NULL;
}

static void emit_load_operand(const char *reg, const char *operand, char **out)
{
    if (is_integer_literal(operand))
        append_linef(out, "    iLoad:  %s <- %s", reg, operand);
    else if (is_float_reg(reg) || is_float_literal(operand))
        append_linef(out, "    load.d: %s <- %s", reg, operand);
    else
        append_linef(out, "    load:   %s <- %s", reg, operand);
}

static void emit_store_symbol(const char *symbol, const char *reg, char **out)
{
    if (is_float_reg(reg))
        append_linef(out, "    store.d: %s <- %s", symbol, reg);
    else
        append_linef(out, "    store:  %s <- %s", symbol, reg);
}

static void emit_move_value(const char *dst_reg, const char *src_reg, char **out)
{
    if (is_float_reg(dst_reg) || is_float_reg(src_reg))
        append_linef(out, "    move.d: %s <- %s", dst_reg, src_reg);
    else
        append_linef(out, "    move:   %s <- %s", dst_reg, src_reg);
}

typedef struct TempRegMap
{
    char name[32];
    char reg[8];
    Data_Type type;
    struct TempRegMap *next;
} TempRegMap;

typedef struct
{
    TempRegMap *head;
    char procedure_name[128];
    int has_procedure_name;
} RtlState;

static const char *k_int_temp_regs[] = {
    "t0", "t1", "t2", "t3", "t4", "t5", "t6", "t7", "t8", "t9",
    "s0", "s1", "s2", "s3", "s4", "s5", "s6", "s7"};

static const char *k_float_temp_regs[] = {
    "f2", "f4", "f6", "f8", "f10", "f12", "f14", "f16", "f18", "f20", "f22", "f24", "f26", "f28", "f30"};

static int is_temp_name(const char *s)
{
    size_t i = 0;

    if (!s)
        return 0;

    if (strncmp(s, "temp", 4) != 0)
        return 0;

    i = 4;
    if (!isdigit((unsigned char)s[i]))
        return 0;

    for (; s[i] != '\0'; i++)
    {
        if (!isdigit((unsigned char)s[i]))
            return 0;
    }

    return 1;
}

static const char *lookup_temp_reg(RtlState *state, const char *name)
{
    TempRegMap *cur;

    if (!state || !name)
        return NULL;

    cur = state->head;
    while (cur)
    {
        if (strcmp(cur->name, name) == 0)
            return cur->reg;
        cur = cur->next;
    }

    return NULL;
}

static Data_Type lookup_temp_type(RtlState *state, const char *name)
{
    TempRegMap *cur;

    if (!state || !name)
        return INT_TYPE;

    cur = state->head;
    while (cur)
    {
        if (strcmp(cur->name, name) == 0)
            return cur->type;
        cur = cur->next;
    }

    return INT_TYPE;
}

static Data_Type lookup_symbol_type_safe(const char *name)
{
    Symbol_Table_Entry *entry;

    if (!name || is_temp_name(name) || is_integer_literal(name) || is_float_literal(name) || is_string_literal(name))
        return INT_TYPE;

    entry = lookup_symbol((char *)name);
    if (!entry)
        return INT_TYPE;
    return entry->type;
}

static Data_Type infer_operand_type(const char *operand, RtlState *state)
{
    if (!operand)
        return INT_TYPE;
    if (is_integer_literal(operand))
        return INT_TYPE;
    if (is_float_literal(operand))
        return FLOAT_TYPE;
    if (is_temp_name(operand))
        return lookup_temp_type(state, operand);

    return lookup_symbol_type_safe(operand);
}

static const char *assign_temp_reg(RtlState *state, const char *name, Data_Type type)
{
    TempRegMap *node;
    TempRegMap *cur;
    size_t i;
    int in_use;
    const char *reg_name = "t0";
    const char **reg_pool = k_int_temp_regs;
    size_t reg_count = sizeof(k_int_temp_regs) / sizeof(k_int_temp_regs[0]);

    if (!state || !name)
        return "t0";

    if (type == FLOAT_TYPE)
    {
        reg_pool = k_float_temp_regs;
        reg_count = sizeof(k_float_temp_regs) / sizeof(k_float_temp_regs[0]);
        reg_name = "f2";
    }

    // if (lookup_temp_reg(state, name))
    //     return lookup_temp_reg(state, name);

    for (i = 0; i < reg_count; i++)
    {
        reg_name = reg_pool[i];
        in_use = 0;
        cur = state->head;
        while (cur)
        {
            if (strcmp(cur->reg, reg_name) == 0)
            {
                in_use = 1;
                break;
            }
            cur = cur->next;
        }
        if (!in_use)
            break;
    }

    node = (TempRegMap *)malloc(sizeof(TempRegMap));
    if (!node)
    {
        fprintf(stderr, "Out of memory in RTL generator\n");
        exit(1);
    }

    snprintf(node->name, sizeof(node->name), "%s", name);
    snprintf(node->reg, sizeof(node->reg), "%s", reg_name);
    node->type = type;
    node->next = state->head;
    state->head = node;
    return node->reg;
}

static void clear_all_temp_regs(RtlState *state)
{
    TempRegMap *cur;

    if (!state)
        return;

    cur = state->head;
    while (cur)
    {
        TempRegMap *next = cur->next;
        free(cur);
        cur = next;
    }

    state->head = NULL;
}

static void remove_temp_reg(RtlState *state, const char *name)
{
    TempRegMap *cur;
    TempRegMap *prev = NULL;

    if (!state || !name)
        return;

    cur = state->head;
    while (cur)
    {
        if (strcmp(cur->name, name) == 0)
        {
            if (prev)
                prev->next = cur->next;
            else
                state->head = cur->next;
            free(cur);
            return;
        }
        prev = cur;
        cur = cur->next;
    }
}

static void release_temp_if_used(RtlState *state, const char *name)
{
    if (!state || !is_temp_name(name))
        return;

    remove_temp_reg(state, name);
}

static const char *upsert_temp_reg(RtlState *state, const char *name, const char *reg)
{
    TempRegMap *cur;
    TempRegMap *node;

    if (!state || !name || !reg)
        return "t0";

    cur = state->head;
    while (cur)
    {
        if (strcmp(cur->name, name) == 0)
        {
            snprintf(cur->reg, sizeof(cur->reg), "%s", reg);
            cur->type = is_float_reg(reg) ? FLOAT_TYPE : INT_TYPE;
            return cur->reg;
        }
        cur = cur->next;
    }

    node = (TempRegMap *)malloc(sizeof(TempRegMap));
    if (!node)
    {
        fprintf(stderr, "Out of memory in RTL generator\n");
        exit(1);
    }

    snprintf(node->name, sizeof(node->name), "%s", name);
    snprintf(node->reg, sizeof(node->reg), "%s", reg);
    node->type = is_float_reg(reg) ? FLOAT_TYPE : INT_TYPE;
    node->next = state->head;
    state->head = node;

    return node->reg;
}

static void free_temp_reg_map(RtlState *state)
{
    clear_all_temp_regs(state);
}

static const char *emit_operand_as_reg(const char *preferred_reg, const char *operand, Data_Type expected_type, RtlState *state, char **out)
{
    const char *mapped = NULL;

    (void)expected_type;

    if (is_temp_name(operand))
    {
        mapped = lookup_temp_reg(state, operand);
        if (mapped)
            return mapped;
    }

    emit_load_operand(preferred_reg, operand, out);
    return preferred_reg;
}

static const char *pick_second_operand_reg(const char *first_reg, const char *result_reg, int is_float)
{
    if (is_float)
    {
        if (strcmp("f2", first_reg) != 0 && strcmp("f2", result_reg) != 0)
            return "f2";
        if (strcmp("f4", first_reg) != 0 && strcmp("f4", result_reg) != 0)
            return "f4";
        if (strcmp("f6", first_reg) != 0 && strcmp("f6", result_reg) != 0)
            return "f6";
        return "f8";
    }

    if (strcmp("t0", first_reg) != 0 && strcmp("t0", result_reg) != 0)
        return "t0";
    if (strcmp("t1", first_reg) != 0 && strcmp("t1", result_reg) != 0)
        return "t1";
    if (strcmp("t2", first_reg) != 0 && strcmp("t2", result_reg) != 0)
        return "t2";
    return "t3";
}

static const char *pick_result_reg(const char *lhs_reg, const char *rhs_reg, int is_float)
{
    if (is_float)
    {
        if (strcmp("f2", lhs_reg) != 0 && strcmp("f2", rhs_reg) != 0)
            return "f2";
        if (strcmp("f4", lhs_reg) != 0 && strcmp("f4", rhs_reg) != 0)
            return "f4";
        return "f6";
    }

    if (strcmp("t0", lhs_reg) != 0 && strcmp("t0", rhs_reg) != 0)
        return "t0";
    if (strcmp("t1", lhs_reg) != 0 && strcmp("t1", rhs_reg) != 0)
        return "t1";
    return "t2";
}

static const char *pick_distinct_float_reg(const char *a, const char *b)
{
    size_t i;
    size_t count = sizeof(k_float_temp_regs) / sizeof(k_float_temp_regs[0]);

    for (i = 0; i < count; i++)
    {
        const char *r = k_float_temp_regs[i];
        if (strcmp(r, a) != 0 && strcmp(r, b) != 0)
            return r;
    }

    return "f30";
}

static const char *pick_distinct_float_reg3(const char *a, const char *b, const char *c)
{
    size_t i;
    size_t count = sizeof(k_float_temp_regs) / sizeof(k_float_temp_regs[0]);

    for (i = 0; i < count; i++)
    {
        const char *r = k_float_temp_regs[i];
        if (strcmp(r, a) != 0 && strcmp(r, b) != 0 && strcmp(r, c) != 0)
            return r;
    }

    return "f30";
}

static const char *arith_to_rtl(const char *op, int is_float)
{
    if (strcmp(op, "+") == 0)
        return is_float ? "add.d" : "add";
    if (strcmp(op, "-") == 0)
        return is_float ? "sub.d" : "sub";
    if (strcmp(op, "*") == 0)
        return is_float ? "mul.d" : "mul";
    if (strcmp(op, "/") == 0)
        return is_float ? "div.d" : "div";
    if (strcmp(op, "&&") == 0)
        return "and";
    if (strcmp(op, "||") == 0)
        return "or";
    return NULL;
}

static const char *relop_to_rtl(const char *op, int is_float)
{
    if (strcmp(op, "<") == 0)
        return is_float ? "slt.d" : "slt";
    if (strcmp(op, "<=") == 0)
        return is_float ? "sle.d" : "sle";
    if (strcmp(op, ">") == 0)
        return is_float ? "sle.d" : "sgt";
    if (strcmp(op, ">=") == 0)
        return is_float ? "slt.d" : "sge";
    if (strcmp(op, "==") == 0)
        return is_float ? "seq.d" : "seq";
    if (strcmp(op, "!=") == 0)
        return is_float ? "seq.d" : "sne";
    return NULL;
}

static int float_relop_uses_movf(const char *op)
{
    return (strcmp(op, ">") == 0 || strcmp(op, ">=") == 0 || strcmp(op, "!=") == 0);
}

static void emit_float_relop_assign(const char *dst_reg, const char *lhs_reg, const char *op, const char *rhs_reg, char **out)
{
    const char *cmp_op = relop_to_rtl(op, 1);
    int use_movf = float_relop_uses_movf(op);

    append_linef(out, "    %s:    %s, %s", cmp_op, lhs_reg, rhs_reg);
    append_linef(out, "    iLoad:  v0 <- 1");
    append_linef(out, "    move:   %s <- zero", dst_reg);
    append_linef(out, "    %s:   %s <- v0, 0", use_movf ? "movf" : "movt", dst_reg);
}

static void emit_print(const char *value, RtlState *state, char **out)
{
    char local[384];
    const char *mapped_temp = NULL;

    snprintf(local, sizeof(local), "%s", value ? value : "");
    trim(local);

    if (is_string_literal(local))
    {
        const char *label = intern_string_label(local);
        append_linef(out, "    iLoad:  v0 <- 4        ;; syscall: print string");
        append_linef(out, "    load_addr: a0 <- %s", label);
        append_linef(out, "    write");
        return;
    }

    append_linef(out, "    iLoad:  v0 <- 1        ;; syscall: print integer");
    if (is_integer_literal(local))
        append_linef(out, "    iLoad:  a0 <- %s", local);
    else if (is_temp_name(local) && (mapped_temp = lookup_temp_reg(state, local)) != NULL)
        append_linef(out, "    move:   a0 <- %s", mapped_temp);
    else
        append_linef(out, "    load:   a0 <- %s", local);
    append_linef(out, "    write");
}

static void emit_copy_assign(const char *dst, const char *rhs, RtlState *state, char **out)
{
    Data_Type rhs_type = infer_operand_type(rhs, state);
    Data_Type dst_type = infer_operand_type(dst, state);
    const char *rhs_pref = (rhs_type == FLOAT_TYPE) ? "f2" : "v0";
    const char *rhs_reg = emit_operand_as_reg(rhs_pref, rhs, rhs_type, state, out);

    if (is_temp_name(dst))
    {
        Data_Type temp_type = (rhs_type == FLOAT_TYPE || dst_type == FLOAT_TYPE) ? FLOAT_TYPE : rhs_type;
        const char *dst_reg = assign_temp_reg(state, dst, temp_type);

        if (strcmp(dst_reg, rhs_reg) != 0)
            emit_move_value(dst_reg, rhs_reg, out);
        if (strcmp(dst, rhs) != 0)
            release_temp_if_used(state, rhs);
        return;
    }

    emit_store_symbol(dst, rhs_reg, out);
    release_temp_if_used(state, rhs);
    clear_all_temp_regs(state);
}

static void emit_unary_assign(const char *dst, const char *op, const char *rhs, RtlState *state, char **out)
{
    Data_Type rhs_type = infer_operand_type(rhs, state);
    int is_float_unary = (rhs_type == FLOAT_TYPE && strcmp(op, "-") == 0);
    const char *src_reg = emit_operand_as_reg(is_float_unary ? "f2" : "v0", rhs, rhs_type, state, out);
    const char *dst_reg = NULL;

    if (is_temp_name(dst))
    {
        if (strcmp(op, "!") == 0)
            dst_reg = upsert_temp_reg(state, dst, "v0");
        else
            dst_reg = assign_temp_reg(state, dst, is_float_unary ? FLOAT_TYPE : INT_TYPE);
    }
    else
    {
        dst_reg = pick_result_reg(src_reg, src_reg, is_float_unary);
    }

    if (is_float_unary && strcmp(dst_reg, src_reg) == 0)
    {
        const char *alt_reg = pick_result_reg(src_reg, src_reg, 1);
        if (is_temp_name(dst))
            dst_reg = upsert_temp_reg(state, dst, alt_reg);
        else
            dst_reg = alt_reg;
    }

    if (strcmp(op, "-") == 0)
        append_linef(out, "    %s: %s <- %s", is_float_unary ? "uminus.d" : "uminus", dst_reg, src_reg);
    else
        append_linef(out, "    not:    %s <- %s", dst_reg, src_reg);

    if (strcmp(dst, rhs) != 0)
        release_temp_if_used(state, rhs);

    if (!is_temp_name(dst))
    {
        emit_store_symbol(dst, dst_reg, out);
        clear_all_temp_regs(state);
    }
}

static void emit_binary_assign(const char *dst, const char *lhs, const char *op, const char *rhs, RtlState *state, char **out)
{
    Data_Type lhs_type = infer_operand_type(lhs, state);
    Data_Type rhs_type = infer_operand_type(rhs, state);
    int is_relop;
    int use_float_op;
    Data_Type result_type;
    const char *rtl_op;
    const char *lhs_reg;
    const char *rhs_reg;
    const char *dst_reg;
    const char *rhs_scratch;

    is_relop = (strcmp(op, "<") == 0 || strcmp(op, "<=") == 0 || strcmp(op, ">") == 0 || strcmp(op, ">=") == 0 || strcmp(op, "==") == 0 || strcmp(op, "!=") == 0);
    use_float_op = (lhs_type == FLOAT_TYPE || rhs_type == FLOAT_TYPE);
    result_type = is_relop ? BOOL_TYPE : (use_float_op ? FLOAT_TYPE : INT_TYPE);

    rtl_op = arith_to_rtl(op, use_float_op);
    if (!rtl_op)
        rtl_op = relop_to_rtl(op, use_float_op);

    if (!rtl_op)
    {
        emit_copy_assign(dst, rhs, state, out);
        return;
    }

    lhs_reg = emit_operand_as_reg(use_float_op ? "f2" : "v0", lhs, lhs_type, state, out);
    if (is_temp_name(dst))
    {
        dst_reg = assign_temp_reg(state, dst, (result_type == FLOAT_TYPE) ? FLOAT_TYPE : INT_TYPE);
    }
    else
    {
        dst_reg = pick_result_reg(lhs_reg, lhs_reg, result_type == FLOAT_TYPE);
    }

    if (result_type == FLOAT_TYPE)
    {
        const char *ordered_dst = pick_result_reg(lhs_reg, lhs_reg, 1);
        if (is_temp_name(dst))
            dst_reg = upsert_temp_reg(state, dst, ordered_dst);
        else
            dst_reg = ordered_dst;
    }

    rhs_scratch = pick_second_operand_reg(lhs_reg, dst_reg, use_float_op);
    rhs_reg = emit_operand_as_reg(rhs_scratch, rhs, rhs_type, state, out);

    if (result_type == FLOAT_TYPE && (strcmp(rhs_reg, lhs_reg) == 0 || strcmp(rhs_reg, dst_reg) == 0))
    {
        const char *rhs_safe = pick_distinct_float_reg3(lhs_reg, dst_reg, rhs_reg);
        emit_move_value(rhs_safe, rhs_reg, out);
        rhs_reg = rhs_safe;
    }
    else if (!is_temp_name(dst))
    {
        dst_reg = pick_result_reg(lhs_reg, rhs_reg, 0);
    }

    if (is_relop && use_float_op)
        emit_float_relop_assign(dst_reg, lhs_reg, op, rhs_reg, out);
    else
        append_linef(out, "    %s:    %s <- %s, %s", rtl_op, dst_reg, lhs_reg, rhs_reg);
    if (strcmp(dst, lhs) != 0)
        release_temp_if_used(state, lhs);
    if (strcmp(dst, rhs) != 0)
        release_temp_if_used(state, rhs);
    if (!is_temp_name(dst))
    {
        emit_store_symbol(dst, dst_reg, out);
        clear_all_temp_regs(state);
    }
}

static void parse_assign_line(const char *buf, RtlState *state, char **out)
{
    char dst[128];
    char rhs[384];
    char a[128], b[128], op[8];
    char unary_rhs[256];

    if (sscanf(buf, "%127s = %383[^\n]", dst, rhs) != 2)
        return;

    trim(dst);
    trim(rhs);

    if (rhs[0] == '!' || rhs[0] == '-')
    {
        snprintf(unary_rhs, sizeof(unary_rhs), "%s", rhs + 1);
        trim(unary_rhs);
        emit_unary_assign(dst, (rhs[0] == '!') ? "!" : "-", unary_rhs, state, out);
        return;
    }

    if (sscanf(rhs, "%127s %7s %127s", a, op, b) == 3)
    {
        emit_binary_assign(dst, a, op, b, state, out);
        return;
    }

    emit_copy_assign(dst, rhs, state, out);
}

static int parse_if_goto(const char *buf, char *cond, size_t cond_size, char *label, size_t label_size)
{
    const char *p = buf;
    const char *start;
    size_t len;

    if (!buf || !cond || !label || cond_size == 0 || label_size == 0)
        return 0;

    if (strncmp(p, "if", 2) != 0)
        return 0;

    p += 2;
    if (*p != '\0' && !isspace((unsigned char)*p) && *p != '(')
        return 0;

    while (*p && isspace((unsigned char)*p))
        p++;

    if (*p == '(')
    {
        p++;
        start = p;
        while (*p && *p != ')')
            p++;
        if (*p != ')')
            return 0;

        len = (size_t)(p - start);
        if (len == 0 || len >= cond_size)
            return 0;

        memcpy(cond, start, len);
        cond[len] = '\0';
        p++;
    }
    else
    {
        start = p;
        while (*p && !isspace((unsigned char)*p))
            p++;

        len = (size_t)(p - start);
        if (len == 0 || len >= cond_size)
            return 0;

        memcpy(cond, start, len);
        cond[len] = '\0';
    }

    while (*p && isspace((unsigned char)*p))
        p++;

    if (strncmp(p, "goto", 4) != 0)
        return 0;
    p += 4;

    if (*p != '\0' && !isspace((unsigned char)*p))
        return 0;

    while (*p && isspace((unsigned char)*p))
        p++;

    start = p;
    while (*p && !isspace((unsigned char)*p) && *p != ';')
        p++;

    len = (size_t)(p - start);
    if (len == 0 || len >= label_size)
        return 0;

    memcpy(label, start, len);
    label[len] = '\0';

    return 1;
}

static int parse_procedure_header(const char *buf, char *proc_name, size_t proc_name_size)
{
    if (!buf || !proc_name || proc_name_size == 0)
        return 0;

    if (sscanf(buf, "proc %127s begin", proc_name) == 1)
        return 1;

    if (sscanf(buf, "**PROCEDURE: %127s", proc_name) == 1)
        return 1;

    if (sscanf(buf, "*PROCEDURE: %127s", proc_name) == 1)
        return 1;

    return 0;
}

static void parse_tac_line(const char *line, RtlState *state, char **out)
{
    char buf[512];
    char cond[128], label[128], dst[128];
    char proc_name[128];
    const char *cond_reg = NULL;

    if (!line)
        return;

    snprintf(buf, sizeof(buf), "%s", line);
    trim(buf);

    if (!*buf)
        return;

    if (parse_procedure_header(buf, proc_name, sizeof(proc_name)))
    {
        snprintf(state->procedure_name, sizeof(state->procedure_name), "%s", proc_name);
        state->has_procedure_name = 1;
        return;
    }

    if (strncmp(buf, "**BEGIN", 7) == 0 || strncmp(buf, "**END", 5) == 0 ||
        strncmp(buf, "*BEGIN", 6) == 0 || strncmp(buf, "*END", 4) == 0)
    {
        return;
    }

    if (strcmp(buf, "return") == 0)
    {
        append_linef(out, "    return");
        return;
    }

    if (buf[strlen(buf) - 1] == ':')
    {
        append_linef(out, "%s", buf);
        return;
    }

    if (sscanf(buf, "goto %127s", label) == 1)
    {
        append_linef(out, "    goto:   %s", label);
        return;
    }

    if (parse_if_goto(buf, cond, sizeof(cond), label, sizeof(label)))
    {
        cond_reg = emit_operand_as_reg("v0", cond, INT_TYPE, state, out);
        append_linef(out, "    bgtz:   %s, %s", cond_reg, label);
        release_temp_if_used(state, cond);
        return;
    }

    if (sscanf(buf, "read %127s", dst) == 1)
    {
        Data_Type dst_type = lookup_symbol_type_safe(dst);

        if (dst_type == FLOAT_TYPE)
        {
            append_linef(out, "    iLoad:  v0 <- 6        ;; syscall: read float");
            append_linef(out, "    read");
            append_linef(out, "    store.d: %s <- f0", dst);
        }
        else
        {
            append_linef(out, "    iLoad:  v0 <- 5        ;; syscall: read integer");
            append_linef(out, "    read");
            append_linef(out, "    store:  %s <- v0", dst);
        }
        return;
    }

    if (strncmp(buf, "print ", 6) == 0)
    {
        emit_print(buf + 6, state, out);
        release_temp_if_used(state, buf + 6);
        return;
    }

    parse_assign_line(buf, state, out);
}

static char *tac_to_rtl(Ast *root, char *procedure_name, size_t procedure_name_size)
{
    FILE *fp;
    char line[512];
    char *rtl = NULL;
    RtlState state = {NULL, "", 0};
    char **lines = NULL;
    size_t line_count = 0;
    size_t line_cap = 0;
    size_t i;

    fp = tmpfile();
    if (!fp)
        return xstrdup("");

    tac_generate(root, fp);
    rewind(fp);

    while (fgets(line, sizeof(line), fp))
    {
        if (line_count == line_cap)
        {
            size_t new_cap = (line_cap == 0) ? 64 : line_cap * 2;
            char **tmp = (char **)realloc(lines, new_cap * sizeof(char *));
            if (!tmp)
            {
                fprintf(stderr, "Out of memory in RTL generator\n");
                exit(1);
            }
            lines = tmp;
            line_cap = new_cap;
        }

        lines[line_count++] = xstrdup(line);
    }

    for (i = 0; i < line_count; i++)
        parse_tac_line(lines[i], &state, &rtl);

    fclose(fp);

    if (procedure_name && procedure_name_size > 0)
    {
        if (state.has_procedure_name)
            snprintf(procedure_name, procedure_name_size, "%s", state.procedure_name);
        else
            procedure_name[0] = '\0';
    }

    free_temp_reg_map(&state);

    for (i = 0; i < line_count; i++)
        free(lines[i]);
    free(lines);

    if (!rtl)
        rtl = xstrdup("");
    return rtl;
}

void rtl_reset_counters(void)
{
    next_string_id = 0;
    free_string_map();
}

void rtl_generate(Ast *root, FILE *out)
{
    char *code;
    char procedure_name[128] = "";

    if (!out)
        return;

    rtl_reset_counters();
    code = tac_to_rtl(root, procedure_name, sizeof(procedure_name));

    if (procedure_name[0] != '\0')
    {
        fputs("*PROCEDURE: ", out);
        fputs(procedure_name, out);
        fputs("\n", out);
    }

    fputs("**BEGIN: RTL Statements\n\n", out);
    fputs(code, out);
    fputs("\n**END: RTL Statements\n", out);

    free(code);
}

int rtl_generate_to_path(Ast *root, const char *path)
{
    FILE *fp;

    if (!path)
        return 0;

    fp = fopen(path, "w");
    if (!fp)
        return 0;

    rtl_generate(root, fp);
    fclose(fp);
    return 1;
}
