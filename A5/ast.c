...
static int check_if_else_stmt_ast(Ast *ast);
static int check_while_ast(Ast *ast);
static int check_call_ast(Ast *ast);
...
static Data_Type current_return_type = VOID_TYPE;
...
static void init_ast_base(Ast *base, Ast_Kind kind, int line, Ast_Check_Fn check_fn, Ast_Print_Fn print_fn)
{
    base->kind = kind;
    base->lineno = line;
    base->check_ast = check_fn;
    base->print = print_fn;
    base->tac_code = NULL;
    base->tac_place = NULL;
}
...
Ast *make_assignment_ast(Ast *lhs, Ast *rhs, int line)
{
    ...
    init_ast_base(&node->base, AST_ASSIGN, line, check_assignment_ast, print_assignment_ast);
...
Ast *make_name_ast(Symbol_Table_Entry *entry, int line)
{
    ...
    init_ast_base(&node->base, AST_NAME, line, check_name_ast, print_name_ast);
...
Ast *make_number_ast(char *value, Data_Type dt, int line)
{
    ...
    init_ast_base(&node->base, AST_NUMBER, line, check_number_ast, print_number_ast);
...
Ast *make_binary_ast(Ast_Kind kind, Ast *l, Ast *r, int line)
{
    ...
    init_ast_base(&node->base, kind, line, check_binary_ast, print_binary_ast);
...
Ast *make_unary_ast(Ast_Kind kind, Ast *child, int line)
{
    ...
    init_ast_base(&node->base, kind, line, check_unary_ast, print_unary_ast);
...
Ast *make_relational_ast(Ast *l, Relational_Op op, Ast *r, int line)
{
    ...
    init_ast_base(&node->base, AST_RELATIONAL, line, check_relational_ast, print_relational_ast);
...
Ast *make_logical_ast(Ast *l, Logical_Op op, Ast *r, int line)
{
    ...
    init_ast_base(&node->base, AST_LOGICAL, line, check_logical_ast, print_logical_ast);
...
Ast *make_if_ast(Ast *cond, Ast *then_p, Ast *else_p, int line)
{
    ...
    init_ast_base(&node->base, AST_IF, line, check_if_ast, print_if_ast);
...
{
    If_Else_Stmt_Ast *node = checked_malloc(sizeof(If_Else_Stmt_Ast));

    init_ast_base(&node->base, AST_IF_ELSE_STMT, line, check_if_else_stmt_ast, print_if_else_stmt_ast);
...
Ast *make_while_ast(Ast *cond, Ast *body, int is_do, int line)
{
    ...
    init_ast_base(&node->base, is_do ? AST_DO_WHILE : AST_WHILE, line, check_while_ast, print_while_ast);
...
{
    Call_Ast *node = checked_malloc(sizeof(Call_Ast));

    init_ast_base(&node->base, AST_CALL, line, check_call_ast, print_call_ast);
    node->base.data_type = VOID_TYPE;

    node->name = strdup(name ? name : "");
    node->args = args;
    node->used_as_statement = 0;
...
Ast *make_read_ast(Ast *var, int line)
{
    ...
    init_ast_base(&node->base, AST_READ, line, check_read_ast, print_read_ast);
...
Ast *make_print_ast(Ast *expr, int line)
{
    ...
    init_ast_base(&node->base, AST_PRINT, line, check_print_ast, print_print_ast);
...
Ast *make_sequence_ast(int line)
{
    ...
    init_ast_base(&node->base, AST_SEQUENCE, line, check_sequence_ast, print_sequence_ast);
...
Ast *make_procedure_ast(char *name, Data_Type return_type, Ast_List *params, int has_body, Ast *body, int line)
{
    ...
    init_ast_base(&node->base, AST_PROCEDURE, line, check_procedure_ast, print_procedure_ast);
...
Ast *make_program_ast(int line)
{
    ...
    init_ast_base(&node->base, AST_PROGRAM, line, check_program_ast, print_program_ast);
...
static int check_if_else_stmt_ast(Ast *ast)
{
    If_Else_Stmt_Ast *node = (If_Else_Stmt_Ast *)ast;
    int ok = 1;
...
static int check_call_ast(Ast *ast)
{
    Call_Ast *node = (Call_Ast *)ast;
    Function_Entry *fn = lookup_function(node->name);
    int ok = 1;

    if (!fn)
    {
        return type_error(ast->lineno, "call to undeclared function");
    }

    if (node->used_as_statement && fn->return_type != VOID_TYPE)
        ok = type_error(ast->lineno, "non-void function return value ignored");

    Ast_List *arg = node->args;
    Param_Type_List *param = fn->params;

    while (arg && param)
    {
        if (!check_ast(arg->stmt))
            ok = 0;
        if (arg->stmt->data_type != param->type)
            ok = type_error(ast->lineno, "function argument type mismatch");
        arg = arg->next;
        param = param->next;
    }

    if (arg || param)
        ok = type_error(ast->lineno, "function argument count mismatch");

    ast->data_type = fn->return_type;
    return ok;
}

static int check_return_ast(Ast *ast)
{
    Return_Ast *node = (Return_Ast *)ast;
    int ok = 1;

    if (node->expr && !check_ast(node->expr))
        ok = 0;
    ...
    {
        if (node->expr)
            return type_error(ast->lineno, "void function should not return a value");
    }
    else
    {
        if (!node->expr)
            return type_error(ast->lineno, "non-void function must return a value");
        if (node->expr && node->expr->data_type != current_return_type)
            return type_error(ast->lineno, "return type mismatch");
    }
...
static int contains_return(Ast *ast)
{
    if (!ast)
        return 0;
    ...
    {
        If_Else_Stmt_Ast *stmt = (If_Else_Stmt_Ast *)ast;
        if (contains_return(stmt->then_part))
            return 1;
        if (stmt->else_part && contains_return(stmt->else_part))
            return 1;
        return 0;
    }
    case AST_WHILE:
    case AST_DO_WHILE:
    {
        ...
        return contains_return(wh->body);
    }
    default:
        return 0;
    }
...
static int check_procedure_ast(Ast *ast)
{
    ...
    if (node->return_type != VOID_TYPE && !contains_return(node->body))
        ok = type_error(ast->lineno, "non-void function must have a return statement");

    in_function = saved_in_function;
    current_return_type = saved_type;
    ast->data_type = VOID_TYPE;
    return ok;
}
