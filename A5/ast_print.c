...
void print_call_ast(Ast *ast, FILE *file)
{
    Call_Ast *call = (Call_Ast *)ast;
    ...
    {
        indent_level++;
        Ast_List *arg = call->args;
        while (arg)
        {
            arg->stmt->print(arg->stmt, file);
            arg = arg->next;
        }
        indent_level--;
    }
